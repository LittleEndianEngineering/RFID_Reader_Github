...
void printWakeupCause(bool suppressQuietTimerWake) {
  esp_sleep_wakeup_cause_t cause = esp_sleep_get_wakeup_cause();
  switch (cause) {
    case ESP_SLEEP_WAKEUP_TIMER:
      if (suppressQuietTimerWake) {
        VERBOSE_PRINTLN("[LIGHTSLEEP] Woke up! cause=LED_HEARTBEAT");
        break;
      }
      wake_count_timer++;
...
void lightSleepUntilNextEvent(uint64_t sleep_us) {
  ...
  uint64_t requested_sleep_us = sleep_us;
  bool ledHeartbeatOnlyWake = false;
  unsigned long heartbeatWaitMs = 0;
  if (getLEDHeartbeatNextEventMs(heartbeatWaitMs)) {
    if (heartbeatWaitMs == 0) {
      updateLEDStatus();
      return;
    }

    uint64_t heartbeat_sleep_us = (uint64_t)heartbeatWaitMs * 1000ULL;
    if (heartbeat_sleep_us < sleep_us) {
      sleep_us = heartbeat_sleep_us;
      ledHeartbeatOnlyWake = true;
    }
  }
  ...
  if (ledHeartbeatOnlyWake) {
    VERBOSE_PRINTF("[LIGHTSLEEP] Entering LED heartbeat sleep for ~%llu ms (requested ~%llu ms)\n",
                   (unsigned long long)(sleep_us / 1000ULL),
                   (unsigned long long)(requested_sleep_us / 1000ULL));
  } else {
  ...
  printWakeupCause(ledHeartbeatOnlyWake);
  if (ledHeartbeatOnlyWake) {
    updateLEDStatus();
  }
